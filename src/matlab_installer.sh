...
root_check () {
  if [ "$EUID" -ne "0" ] ; then
    whiptail --backtitle "$script" --title "$program" --msgbox "ERROR: ROOT PRIVILEGES ARE REQUIRED TO CONTINUE. EXITING." >&2 10 40
    exit 1
fi
}
...
check_disk_space () {
  if [ $(df -l --output=avail /usr/local |awk 'FNR == 2 {print $1}') -le "31457280" ]; then
    whiptail --backtitle "$script" --title "$program" --msgbox "ERROR: NOT ENOUGH FREE DISK SPACE. EXITING." >&2 10 40
    exit 1
fi
}
...
curl_check () {
  if [ $(dpkg-query --show --showformat='${Status}' curl 2>/dev/null | grep --count "ok installed") -eq "0" ]; then
    whiptail --backtitle "$script" --title "$program" --msgbox "CURL IS NOT INSTALLED. LET'S INSTALL IT..." >&2 10 40
...
ping_local_web () {
  whiptail --backtitle "$script" --title "$program" --msgbox "PINGING CNS LOCAL WEB..." 10 40

  if ping -c 1 "$LOCAL_WEB" &> /dev/null; then
    whiptail --backtitle "$script" --title "$program" --msgbox "CNS LOCAL WEB IS REACHABLE. CONTINUING..." 10 40 
  else
    whiptail --backtitle "$script" --title "$program" --msgbox "ERROR: CNS LOCAL WEB IS NOT REACHABLE. EXITING." >&2
    exit 1
fi
}
