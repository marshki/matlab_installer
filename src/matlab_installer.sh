...
remove_matlab_tar () {
  dialog --backtitle "$script" --title "$program" --infobox "REMOVING $MATLAB[0]} INSTALLER..."
...
local_bin_check () {
  if [ ! -d "/usr/local/bin" ] ; then
    ...
    dialog --backtitle "$script" --title "$program" --infobox "/usr/local/bin DOES NOT exist; LET'S ADD IT..."
...
symlink_matlab () {
  dialog --backtitle "$script" --title "$program" --infobox "CREATING SYMLINK FOR ${MATLAB[0]}..."
